...
# -*- coding: utf-8 -*-
"""
Generate LaTeX table from edge ablation results.
Automatically highlights best (bold) and second-best (underline) for each metric.
Sorts configurations by average R@1, then R@5, then R@10.
"""
...
def generate_latex_table(results_dir: str, output_path: str):
    ...
    # Sort by avg R@1 (desc), then R@5 (desc), then R@10 (desc)
    # Treat None as -infinity for sorting purposes
    config_data.sort(
        key=lambda x: (
            x['avg']['r@1'] if x['avg']['r@1'] is not None else -float('inf'),
            x['avg']['r@5'] if x['avg']['r@5'] is not None else -float('inf'),
            x['avg']['r@10'] if x['avg']['r@10'] is not None else -float('inf')
        ),
        reverse=True
    )
    ...
    latex_lines.append("\\caption{Recall metrics across different datasets. Best results are \\textbf{bold}, second best are \\underline{underlined}.}")
    ...
    for i in range(7):  # 6 datasets + 1 avg
        end_col = start_col + 2
        latex_lines.append(f"\\cmidrule(lr){{{start_col}-{end_col}}}")
        start_col += 3
    latex_lines.append("")
